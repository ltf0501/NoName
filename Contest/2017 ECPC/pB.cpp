...
int main() {
  ...
  for (int c = 1; c <= cases; c++) {
    ...
    for (int i = 0; i < Q; i++) {
      ...
      x--;
      y--;  // I consider the input  1-based
    ...
    // for each position pos in s1  find the longest substring that end at pos
    // and is substring of s2
    int l = 0, v = 0;
    for (int i = 0; i < n; i++) {
    ...
    for (int i = 1; i < n; i++)
      cnt[i] += cnt[i - 1];  // now for each position in s1 we know how many
                             // substring contains it and is substring in s2
    build(1, 0, n - 1);
    int last = 0;
    for (auto it : q) {
      x = it.first.first;

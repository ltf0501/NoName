...
bool ancestor(int id, int x, int y) {
  return in[id][x] <= in[id][y] && out[id][x] >= out[id][y];
}
int LCA(int id, int x, int y) {
  if (ancestor(id, x, y)) return x;
  if (ancestor(id, y, x)) return y;
  for (int i = 17; i >= 0; i--)
    if (!ancestor(id, anc[id][i][x], y)) x = anc[id][i][x];
  return anc[id][0][x];
}
...
int main() {
  ...
  for (int i = 1; i <= n; i++) {
    for (int j : g[0][i])
      if (i < j) num[1][LCA(1, i, j)] -= 2, num[1][i]++, num[1][j]++;
    for (int j : g[1][i])
      if (i < j) num[0][LCA(0, i, j)] -= 2, num[0][i]++, num[0][j]++;
  }
  dfs2(0, 1, 0);
  for (int i = 2; i <= n; i++) {
    if (num[0][i] == 1) cnt2++;

...
void mult(int *a, int *b, int *res) {
  ...
  for (int i = 0; i < SZ; i++) {
    for (int j = 0; j < SZ; j++) {
      for (int k = 0; k < SZ; k++) {
        res[i * SZ + j] =
            (res[i * SZ + j] + int(1LL * a[i * SZ + k] * b[k * SZ + j] % MOD)) %
            MOD;
      }
    }
  }
...
void pow(int *x, int e, int *res) {
  for (int i = 0; i < SZ; i++)
    for (int j = 0; j < SZ; j++) res[i * SZ + j] = 0;
  ...
  while (e) {
    ...
    if (e & 1) {
      mult(res, x, temp);
      for (int i = 0; i < SZ; i++)
        for (int j = 0; j < SZ; j++) res[i * SZ + j] = temp[i * SZ + j];
    }
    e >>= 1;
    mult(x, x, temp);
    for (int i = 0; i < SZ; i++)
      for (int j = 0; j < SZ; j++) x[i * SZ + j] = temp[i * SZ + j];
  }
  return;
}
int main() {

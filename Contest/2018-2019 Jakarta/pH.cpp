...
struct seg_tree {
  ...
  void fix(int n, int l, int r, int pos, int x) {
    if (l == r) {
      ...
      int mid = (l + r) >> 1;
      if (pos > mid)
        fix(n * 2 + 2, mid + 1, r, pos, x);
      else
        fix(n * 2 + 1, l, mid, pos, x);
      v[n] = v[n * 2 + 1] + v[n * 2 + 2];
      a[n] = a[n * 2 + 1] + a[n * 2 + 2];
    }
  }
...
bool cant(int len, int need) { return need > len; }
int main() {
  ...
  for (int i = 1; i <= n; i++)
    if (a[i] == 0) {
      while (now < k) {
        if (p[now].F.F <= i) {
          if (cant(p[now].F.S - p[now].F.F + 1 -
                       sg.cnt(0, 1, n, p[now].F.F, p[now].F.S),
                   p[now].S - sg.ask(0, 1, n, p[now].F.F, p[now].F.S))) {
            printf("Impossible\n");
            return 0;
          }
          pq.push(
              {pre + can(p[now].F.S - p[now].F.F + 1 -
                             sg.cnt(0, 1, n, p[now].F.F, p[now].F.S),
                         p[now].S - sg.ask(0, 1, n, p[now].F.F, p[now].F.S)),
               p[now].F.S});
          now++;

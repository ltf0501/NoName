...
int main()
{
	...
	while(t--)
	{
		...
		bool flag=1;
		for(int i=1;i<=N;i++)if(finds(i)==i && e[i]>sz[i]){flag=0;break;}
		if(!flag)
		{
			puts("-1 -1");
			continue ;
		...
		for(int i=1;i<=N;i++)if(p[i]==i)
		{
			if(sz[i]==e[i])
			{
				...
				for(int u : vv[i])if(!vis[u]){rt=u,cir_sz++;}
				vector<int> cyc;
				dfs2(rt,cyc);
				int m=(int)cyc.size();
				for(int j=0;j<m;j++)
				{
					...
					for(pii v : g[a])if(v.F==b)cnt+=v.S;
				}
				if(m==1)cnt=0;
				else if(m==2)
				{
					int a=cyc[0],b=cyc[1];
					int c=0;
					for(pii v : g[a])if(v.F==b)c+=v.S;
					if(c==0 || c==2)cnt=1;
					else cnt=0;
				}
				if(cnt==cir_sz-cnt)ans+=cnt,tot=tot*2%mod;
				else ans+=min(cnt,cir_sz-cnt);
			}
			else 
			{
				dfs_clock=0;
				...
				{
					if(mn>pre[j])mn=pre[j],cnt=1;
					else if(mn==pre[j])cnt++;
				}
				ans+=mn;
				tot=tot*cnt%mod;

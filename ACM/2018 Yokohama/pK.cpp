...
bool can(int pos,int need){
	...
	vector<int> temp,r;
	for(int i:v)if(i>=0)temp.PB(i);
	for(int i:m)if(i>=0)r.PB(i);
	if(debug){
		printf("pos=%d need=%d\n",pos,need);
		printf("temp::");for(int i:temp)printf(" %d",i);
		printf("\nr::");for(int i:r)printf(" %d",i);
		printf("\n");
	}
	for(int i=0;i<need;i++)if(temp[n-pos-need+i]<=r[i])return false;
	return true;
}
int main(){
	scanf("%d",&n);
	...
	sort(v.begin(),v.end());
	for(int i=n;i>=0;i--)if(can(0,i)){
		win=i;
		break;
	}
	if(debug)printf("win=%d\n",win);
	for(int i=0;i<n;i++){
		for(int j=0;j<n;j++)if(m[j]==a[i]){
			m[j]=-1;
			break;
		}
		if(v[0]>a[i]){

...
struct P
{
    ...
    bool operator < (const P& rhs) const {return x==rhs.x ? y<rhs.y : x<rhs.x ;}
...
vector<P> ConvexHull(vector<P> p)
{
    ...
    for(int i=1;i<SZ(p);i++)
    {
        while(SZ(res)>=2)
        {
            if(((p[i]-res[SZ(res)-2])^(res[SZ(res)-1]-res[SZ(res)-2]))>=0)res.pop_back(),id.pop_back();
            else break;
        }
...
int main()
{
    ...
    for(int i=1,cur=0;i<n;i++)
    {
        ...
        else
        {
            ...
            if(cur>0)
            {
                tmp.push_back(convex[(cur-1+m)%m]),tmp.push_back(convex[cur]),tmp.push_back(convex[(cur+1)%m]);
                if(inpoly(A[i],tmp))ss[cur].push_back(A[i]);
            }
            tmp.clear();
            if(cur!=m-1)
            {
                tmp.push_back(convex[cur]),tmp.push_back(convex[(cur+1)%m]),tmp.push_back(convex[(cur+2)%m]);
    ...
    vector<P> tmp;
    tmp.push_back(convex[m-1]),tmp.push_back(convex[0]),tmp.push_back(convex[1]);
    ...
    for(int i=0;i<m;i++)
    {
        P a=convex[(i-1+m)%m],b=convex[i],c=convex[(i+1)%m],d=convex[(i+2)%m];
    ...
    for(int i=2;i<m-1;i++)
    {
        P a1=convex[(i-3+m)%m],b1=convex[(i-2+m)%m],c1=convex[(i-1+m)%m];
    ...
    for(int i=0;i<m;i++)
    {
		...
		double sub=0;
		for(int j=1;j<SZ(con)-1;j++)sub=max(sub,dis(con[j-1],con[j])+dis(con[j],con[j+1])-dis(con[j-1],con[j+1]));
		sum-=sub;
        ans3=max(ans3,(dis(convex[(i-1+m)%m],convex[i])+dis(convex[i],convex[(i+1)%m]))-sum);
    }
	printf("ans3 = %.3f\n",ans3);

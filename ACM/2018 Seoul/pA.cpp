...
struct seg_tree{
    ...
    int ask(int n,int l,int r,int L,int R){
        if(L<=l&&r<=R)return v[n];
            ...
            int mid=(l+r)>>1;
            push(n);
            return max(ask(n*2+1,l,mid,L,R),ask(n*2+2,mid+1,r,L,R));
        }
    }
};
int main(){
    ...
    for(int i=0;i<n;i++)p[i]={lower_bound(x.begin(),x.end(),p[i].F)-x.begin(),lower_bound(x.begin(),x.end(),p[i].S)-x.begin()};
    sg.init(0,0,(int)x.size()-1);
    sort(p,p+n);
    for(int i=0;i<n;i++)sg.fix(0,0,(int)x.size()-1,p[i].F,p[i].S,1);

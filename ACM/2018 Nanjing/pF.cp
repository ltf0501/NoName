...
void dc(int l,int r,int lv){
    ...
    if(l==r){
        out(lv);
        for(int i=0;i<n;i++)if(i!=l)dis[i][l]=A[lv][i][n]/A[lv][i][i];
    }
    else{
        int mid=(l+r)>>1;
        double val;
        lv++;
        for(int i=0;i<n;i++)for(int j=0;j<=n;j++)A[lv][i][j]=A[lv-1][i][j];
        for(int i=l;i<=mid;i++){
...
int main(){
    ...
    double ans;
    scanf("%d%d%d",&n,&m,&q);
    for(int i=0;i<=n;i++)for(int j=0;j<=n;j++)A[0][i][j]=0;
    while(m--){
        scanf("%d%d",&l,&r);
        A[0][l][r]-=1;
        A[0][l][l]+=1;
